---
title: "Worklog Tuần 6"
date: 2026-07-06
weight: 6
chapter: false
pre: " <b> 1.6. </b> "
---

### Mục tiêu tuần 6:

* Giám sát tài nguyên AWS bằng Amazon CloudWatch.
* Tìm hiểu Infrastructure as Code với AWS CloudFormation.

### Các công việc cần triển khai trong tuần này:

| Thứ | Công việc | Ngày bắt đầu | Ngày hoàn thành | Nguồn tài liệu |
| --- | --- | --- | --- | --- |
| 2 | Tìm hiểu CloudWatch Metrics và CloudWatch Logs | 06/07/2026 | 06/07/2026 | AWS Documentation |
| 3 | Cấu hình CloudWatch Alarm và thông báo | 07/07/2026 | 07/07/2026 | AWS Documentation |
| 4 | Tìm hiểu cấu trúc và cú pháp của CloudFormation Template | 08/07/2026 | 08/07/2026 | AWS Documentation |
| 5 | Tạo và triển khai CloudFormation Stack | 09/07/2026 | 09/07/2026 | AWS Documentation |
| 6 | Kiểm tra quá trình tự động khởi tạo tài nguyên | 10/07/2026 | 11/07/2026 | AWS Documentation |

### Kết quả đạt được tuần 6:

* Cấu hình thành công hệ thống giám sát bằng Amazon CloudWatch.
* Thiết lập các CloudWatch Alarm để theo dõi tài nguyên.
* Hiểu được khái niệm Infrastructure as Code và cách áp dụng với CloudFormation.
* Triển khai thành công tài nguyên AWS thông qua CloudFormation Stack.
* Nâng cao khả năng tự động hóa việc triển khai và quản lý hạ tầng.