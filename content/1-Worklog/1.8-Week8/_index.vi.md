---
title: "Worklog Tuần 8"
date: 2026-07-20
weight: 8
chapter: false
pre: " <b> 1.8. </b> "
---

### Mục tiêu tuần 8:

* Triển khai Backend và Frontend lên môi trường AWS.
* Xây dựng và vận hành quy trình triển khai tự động (CI/CD).

### Các công việc cần triển khai trong tuần này:

| Thứ | Công việc | Ngày bắt đầu | Ngày hoàn thành | Nguồn tài liệu |
| --- | --- | --- | --- | --- |
| 2 | Triển khai dịch vụ Backend | 20/07/2026 | 20/07/2026 | AWS Documentation |
| 3 | Triển khai ứng dụng Frontend | 21/07/2026 | 21/07/2026 | AWS Documentation |
| 4 | Cấu hình quy trình CI/CD | 22/07/2026 | 22/07/2026 | AWS Documentation |
| 5 | Kiểm thử quy trình triển khai tự động | 23/07/2026 | 23/07/2026 | AWS Documentation |
| 6 | Kiểm tra hoạt động của toàn bộ hệ thống | 24/07/2026 | 25/07/2026 | Tài liệu dự án |

### Kết quả đạt được tuần 8:

* Triển khai thành công dịch vụ Backend lên AWS.
* Triển khai thành công ứng dụng Frontend.
* Cấu hình và vận hành quy trình CI/CD tự động.
* Kiểm thử thành công quá trình triển khai từ đầu đến cuối.
* Đảm bảo hệ thống hoạt động ổn định sau khi triển khai.